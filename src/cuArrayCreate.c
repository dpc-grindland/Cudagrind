...
CUresult I_WRAP_SONAME_FNNAME_ZZ(libcudaZdsoZa, cuArrayCreate)(CUarray *pHandle, const CUDA_ARRAY_DESCRIPTOR *pAllocateArray) {
   OrigFn      fn;
   CUresult    result;
   CUcontext   ctx = NULL;
   ...
   if (result == CUDA_SUCCESS) {
      ...
      cgGetCtx(&ctx);
      // .. and add the freshly created 2D array to the list.
      cgAddArr(ctx, *pHandle);
   } else {
      VALGRIND_PRINTF_BACKTRACE("cuArrayCreate returned with error code: %d\n", result);
   }
