...
/******************************************************************************
 *
 * Wrapper for cudaMemcpyToSymbol
 *
...
cudaError_t I_WRAP_SONAME_FNNAME_ZZ(libcudartZdsoZa, cudaMemcpyToSymbol)(const void *symbol, const void *src, size_t count, size_t offset, enum cudaMemcpyKind kind) {
   OrigFn      fn;
   cudaError_t result;
   CUcontext   ctx = NULL;
   int         errors = 0;
   ...
   switch (kind) {
      ...
      case cudaMemcpyDeviceToDevice: {
         ...
         if (getPtrError == cudaErrorInvalidSymbol || getSizeError == cudaErrorInvalidSymbol) {
            errors++;
            VALGRIND_PRINTF("Error: Invalid symbol in call to cudaMemcpyToSymbol.\n");
         } else {
            ...
            CALL_FN_W_5W(result, fn, symbol, src, count, offset, kind);
         }
         break;
      }
      default: {
         errors++;
         VALGRIND_PRINTF("Error: Invalid kind in call to cudaMemcpyToSymbol\n");
