...
cudaError_t I_WRAP_SONAME_FNNAME_ZZ(libcudartZdsoZa, cudaMemcpyFromSymbolAsync)(void *dst, const void *symbol, size_t count, size_t offset, enum cudaMemcpyKind kind, cudaStream_t stream) {
   OrigFn      fn;
   cudaError_t result;
   CUcontext   ctx = NULL;
   int         errors = 0;
   ...
   if (vgErrorAddress) {
      error++;
      VALGRIND_PRINTF("Error: 'stream' in call to cudaMemcpyFromSymbolAsync not defined.\n");
   }
   ...
   switch (kind) {
      ...
      case cudaMemcpyDeviceToDevice: {
         nodeMemDst = cgFindMem(nodeCtx, (CUdeviceptr)dst);
      ...
      case cudaMemcpyDeviceToHost: {
         ...
         if (getPtrError == cudaErrorInvalidSymbol || getSizeError == cudaErrorInvalidSymbol) {
            ...
            nodeMemSrc = cgFindMem(nodeCtx, (CUdeviceptr)symbolPtr);
            ...
            if (!nodeMemSrc) {
               ...
               nodeMemSrc = cgFindMem(nodeCtx, (CUdeviceptr)symbolPtr);
               nodeMemSrc->isSymbol = 1;
               nodeMemSrc->locked = CG_STREAM_READING;
               nodeMemSrc->stream = (CUstream)stream;
            } else {
            ...
            CALL_FN_W_6W(result, fn, dst, symbol, count, offset, kind, stream);
         }
         break;
      }
      default: {
         errors++;
         VALGRIND_PRINTF("Error: Invalid kind in call to cudaMemcpyFromSymbolAsync.\n");
